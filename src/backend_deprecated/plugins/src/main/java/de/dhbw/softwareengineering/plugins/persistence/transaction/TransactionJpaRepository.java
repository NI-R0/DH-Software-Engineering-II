...
public interface TransactionJpaRepository extends JpaRepository<TransactionJpaEntity, UUID> {
    ...
    @Query(value="SELECT * FROM TRANSACTION WHERE account = ?1 ORDER BY timestamp ASC", nativeQuery = true)
    ...
    @Query(value="DELETE FROM TRANSACTION WHERE account = ?1",nativeQuery = true)
    @Modifying
